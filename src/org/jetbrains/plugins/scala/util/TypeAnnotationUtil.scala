...
object TypeAnnotationUtil {
  ...
  def isTypeAnnotationNeededProperty(property: ScMember)
                                    (implicit settings: ScalaCodeStyleSettings,
                                     pair: (Boolean, Boolean)): Boolean = {
    val visibility = if (isMemberOf(property, "scala.DelayedInit")) Local
    else Visibility(property)

    visibility match {
      case Implicit if settings.IMPLICIT_PROPERTY_TYPE_ANNOTATION == isRequired => true
      case _ => isTypeAnnotationNeededProperty(visibility)
    }
  }
  ...
  def isMemberOf(member: ScMember, fqn: String): Boolean = {
    val maybeScope = Option(getModule(member))
      .map(moduleWithDependenciesAndLibrariesScope)
      .map(ElementScope(member.getProject, _))

    val maybeClass = maybeScope
      .flatMap(_.getCachedClass(fqn))

    Option(member.getContainingClass).zip(maybeClass)
      .exists {
        case (containingClass, clazz) => containingClass.isInheritor(clazz, true)
      }
  }
